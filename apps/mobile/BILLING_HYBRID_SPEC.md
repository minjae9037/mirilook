...
### 7.1 Google Play Console — 일회성 제품 생성 (2026-07-17 실제 수행 기준으로 재작성)

> ⚠️ **옛 내용 폐기.** "수익 창출 > 상품 > 인앱 상품" 경로와 `hair_money_4` 류 ID로 적혀 있었으나
> 둘 다 틀렸다. 구글이 **"인앱 상품" → "일회성 제품"으로 개명**했고 데이터 모델도 바뀌었다.

**메뉴 경로:** 좌측 **`Play를 통한 수익 창출` → `제품` → `일회성 제품`** → **「일회성 제품 만들기」**
(「사용자 늘리기」는 스토어 등록정보 메뉴다 — 여기가 아니다.)

**새 데이터 모델 — ID가 2개다:**
```
제품 (제품 ID = hair_money_2000, 이름, 설명, 세금 카테고리)
  └ 구매 옵션 (구매 옵션 ID = standard, 구매 유형, 가격/지역)   ← 가격이 여기로 내려감
      └ 혜택/프로모션 (선택)
```

**1단계 — 상품 세부정보**
- 제품 ID: §4 표 그대로 (**생성 후 변경·재사용 영구 불가**)
- 이름 / 설명: §4 표
- **제품 세금 카테고리: `디지털 앱 판매`** (기본값)
- 태그·아이콘·연령등급: 비워둬도 됨(선택). 아이콘은 구글이 "제품마다 고유 이미지"를 권장하므로
  6종에 같은 앱 아이콘을 넣지 말고 비워두는 편이 낫다. 나중에 수정 가능.

**2단계 — 구매 옵션 + 가격**
- **구매 옵션 ID: `standard`** (6종 전부 동일. 사용자에게 안 보이는 내부 식별자)
- **구매 유형: `구입`** (「임대」 아님)
- **고급 옵션 → `디지털 콘텐츠`** 선택 (「서비스」 아님 — HM은 앱 내 가상 재화)
- 고급 옵션 → "단일 거래에서 2개 이상 구매 허용"은 **체크하지 말 것** (스토어가 패키지를 1개씩 팔고,
  RevenueCat 구매 호출도 수량 개념을 안 쓴다)

**🔴 여기서 반드시 막힌다 — "변경사항을 저장할 수 없습니다"**

기본값이 **전 세계 200여 개국이 "사용 가능"**인데, 구글은 **사용 가능한 모든 지역에 가격을 요구**한다.
대한민국만 가격을 넣고 활성화를 누르면 저장이 거부되는데, **오류 원인이 화면 밖(스크롤 아래)에 있어
찾기 어렵다.** (2026-07-17 실제로 여기서 막혔다.)

→ **해결 순서:**
1. 「사용 가능 여부 및 액세스 수정」으로 **전체 지역을 먼저 "사용 불가"로** 바꾼다.
2. `대한민국` 검색 → **사용 가능** + 가격 입력.
3. **활성화 누르기 전에** 「모든 지역」 드롭다운을 **"사용 가능" 필터**로 바꿔
   **대한민국 하나만 남았는지 확인**한다. 남은 게 있으면 그게 저장 거부의 원인이다.

**지역을 한국만 두는 이유** (앱을 국가별로 쪼개기 때문이 아니다 — 앱 하나로 전 세계 가능하며 Play가
국가별 등록정보·가격을 기본 지원한다): 서비스가 아직 한국에 묶여 있다. 미용실 찾기는 한국 미용실만,
약관·개인정보·환불은 한국 법 기준, 지원은 한국어, EU 판매 시 GDPR 대응 필요. 또한 **Play 결제 국가는
물리적 위치가 아니라 결제수단 국가를 따르므로 해외 거주 한국인은 한국 계정으로 그대로 구매된다.**
나중에 지역·가격 추가는 같은 제품에 몇 분이면 되고 재심사도 없다.

**✅ 「이전 버전과의 호환성」은 자동이다**

활성화하면 `standard` 옵션에 **「이전 버전과의 호환성」 배지가 자동으로 붙는다.** 이게 있어야
RevenueCat이 **제품 ID로** 상품을 조회할 수 있다(§6-A `getProducts`). 수동 설정은 없다.
⚠️ 나중에 구매 옵션을 추가할 때 이 배지는 **하나에만** 붙는다 — `standard`에서 떼면 앱이 상품을 못 불러온다.

**지급 프로필:** 「일회성 제품」 메뉴가 잠겨 있거나 저장이 안 되면 **수익 창출 설정 > 판매자 계정(지급
프로필)**을 엠제이인사이트 사업자 정보로 먼저 등록해야 한다.

**소모성 처리:** Play Console에 "소모성" 체크박스는 없다. 소비(consume) 여부는 Billing Library에서
결정되고 RevenueCat이 구매 후 자동 consume한다 — 위처럼 일반 일회성 제품으로 만들면 된다.
