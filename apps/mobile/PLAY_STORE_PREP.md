...
**대표님 권장 판단:** 미리룩은 실제로 **미용실 연결(오프라인 상담)** 서비스를 포함하므로, **A(글로벌 표준) 또는 C(실서비스 프레이밍)** 를 저와 함께 검토 권장. **재패키징 전에 이 결정을 먼저 내려야** 앱 결제 코드/스토어 신고 내용이 정해집니다. → PG 연결 완료 시 이 항목부터 같이 정합니다.
